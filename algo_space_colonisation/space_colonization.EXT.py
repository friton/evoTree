...
class space_colonization:
    ...
    def get_closest_tree_points(self,max_x,min_x):
        ...
        Takes:
            x_max and x_min
            the area between x_max and x_min is the area where attraction points can attract active tree points  
        Returns:
...
#        print "reading attraction points from",attr_file
 ...
 #       print "generating 60000 random attr points"
...
#            ------------
#            |  x=10000 |
#            |          |y=500  attr points area dimensions
#            |          |
#            ------------
    ...
    PROBA_XTINCT = [0.99,0.89,0.8,0.7,0.6,0.65,0.85,0.6,0.85,0.75,0.95,0.75,0.5]
    SEUIL_XTINCT = [5000,9000,33000,55000,62000,68000,74000,79000,89767,91395,94186,98488]
    SEUIL_XTINCT_AREA = []
    PROBA_XTINCT_AREA = []
    for i,sx in enumerate(SEUIL_XTINCT): ## get progressive extinctions
          SEUIL_XTINCT_AREA.append(sx-1000)
          SEUIL_XTINCT_AREA.append(sx-900)
          SEUIL_XTINCT_AREA.append(sx-800)
          SEUIL_XTINCT_AREA.append(sx-700)
          SEUIL_XTINCT_AREA.append(sx-600)
          SEUIL_XTINCT_AREA.append(sx-500)
          SEUIL_XTINCT_AREA.append(sx-400)
          SEUIL_XTINCT_AREA.append(sx-300)
          SEUIL_XTINCT_AREA.append(sx-200)
	...
	if j%50 == 0:
        ...
        ## We recover the area in which the attraction points can attract a tree point (outside this area we don't need to check the attraction points)
	pxtocheck = [SC.tree_points[toto].x for toto in  SC.tree_points_to_check]
        max_x = max(pxtocheck)
	min_x = min(pxtocheck)

        
        ## We tell the points they are not live anymore
        for p in Lextinct:
